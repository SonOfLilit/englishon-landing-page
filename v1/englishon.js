...
if($.ui.ddmanager&&!this.options.dropBehaviour){$.ui.ddmanager.drop(this,event);}if(this.options.revert){var that=this,cur=this.placeholder.offset(),axis=this.options.axis,animation={};if(!axis||axis==="x"){animation.left=cur.left-this.offset.parent.left-this.margins.left+(this.offsetParent[0]===this.document[0].body?0:this.offsetParent[0].scrollLeft);}if(!axis||axis==="y"){animation.top=cur.top-this.offset.parent.top-this.margins.top+(this.offsetParent[0]===this.document[0].body?0:this.offsetParent[0].scrollTop);}this.reverting=true;$(this.helper).animate(animation,parseInt(this.options.revert,10)||500,function(){that._clear(event);});}else{this._clear(event,noPropagation);}return false;},cancel:function(){if(this.dragging){this._mouseUp({target:null});if(this.options.helper==="original"){this.currentItem.css(this._storedCSS).removeClass("ui-sortable-helper");}else{this.currentItem.show();}//Post deactivating events to containers
...
items.push({item:item,instance:targetData,width:0,height:0,left:0,top:0});}}},refreshPositions:function(fast){// Determine whether items are being displayed horizontally
...
easing=easing||options.easing||animate.easing;duration=duration||options.duration||animate.duration;if(!toHide.length){return toShow.animate(this.showProps,duration,easing,complete);}if(!toShow.length){return toHide.animate(this.hideProps,duration,easing,complete);}total=toShow.show().outerHeight();toHide.animate(this.hideProps,{duration:duration,easing:easing,step:function(now,fx){fx.now=Math.round(now);}});toShow.hide().animate(this.showProps,{duration:duration,easing:easing,complete:complete,step:function(now,fx){fx.now=Math.round(now);if(fx.prop!=="height"){if(boxSizing==="content-box"){adjust+=fx.now;}}else if(that.options.heightStyle!=="content"){fx.now=Math.round(total-toHide.outerHeight()-adjust);adjust=0;}}});},_toggleComplete:function(data){var toHide=data.oldPanel;toHide.removeClass("ui-accordion-content-active").prev().removeClass("ui-corner-top").addClass("ui-corner-all");// Work around for rendering bug in IE (#5421)
 ...
 */var progressbar=$.widget("ui.progressbar",{version:"1.11.4",options:{max:100,value:0,change:null,complete:null},min:0,_create:function(){// Constrain initial value
 ...
 */var tooltip=$.widget("ui.tooltip",{version:"1.11.4",options:{content:function(){// support: IE<9, Opera in jQuery <1.7
...
if(event&&event.type==="mouseover"){target.parents().each(function(){var parent=$(this),blurEvent;if(parent.data("ui-tooltip-open")){blurEvent=$.Event("blur");blurEvent.target=blurEvent.currentTarget=this;that.close(blurEvent,true);}if(parent.attr("title")){parent.uniqueId();that.parents[this.id]={element:this,title:parent.attr("title")};parent.attr("title","");}});}this._registerCloseHandlers(event,target);this._updateContent(target,event);},_updateContent:function(target,event){var content,contentOption=this.options.content,that=this,eventType=event?event.type:null;if(typeof contentOption==="string"){return this._open(event,target,contentOption);}content=contentOption.call(target[0],function(response){// IE may instantly serve a cached response for ajax requests
...
// event.
if(this.options.show&&this.options.show.delay){delayedShow=this.delayedShow=setInterval(function(){if(tooltip.is(":visible")){position(positionOption.of);clearInterval(delayedShow);}},$.fx.interval);}this._trigger("open",event,{tooltip:tooltip});},_registerCloseHandlers:function(event,target){var events={keyup:function(event){if(event.keyCode===$.ui.keyCode.ESCAPE){var fakeEvent=$.Event(event);fakeEvent.currentTarget=target[0];this.close(fakeEvent,true);}}};// Only bind remove handler for delegated targets. Non-delegated
...
Authenticator.prototype.getToken = function () {
  ...
  console.log("getToken****open the login dialog");
...
Authenticator.prototype.login = function (token) {
  ...
  console.log('Authenticator.prototype.login****There is no token. Create new one for guest');
      ...
      if (isExist == false) {
        ...
        if (match[0] in this.internal_id) {
          ...
          for (var i = 0; i < this.internal_id[match[0]].length; i++) {
            select.append($('<option>').text(this.internal_id[match[0]][i]));
          }
          ...
          for (var i = 0; i < this.internal_id[match[0].slice(1)].length; i++) {
            select.append($('<option>').text(this.internal_id[match[0].slice(1)][i]));
          }
...
Injector = function (paragraphs) {
  ...
  this.findTarget = function (ctx, replaced) {
    ...
    $.each(paragraphs, function (i, p) {
      ...
      findAndReplaceDOMText(p, { find: ctx,
...
var AbstractQuestion = function (qdata, toggleSound) {
  ...
  this.practicedWord = this.data.hint_language === document.config.targetLanguage ? this.data.hint : this.correct[0];
...
AbstractQuestion.prototype.touch = function (event) {
  if (!this.touched) {
    ...
    var language = document.config.targetLanguage;
...
AbstractQuestion.prototype.guess = function (answer) {
  //console.log("EVENT guess ");
  var isAnswerInTargetLanguage = this.practicedWord !== this.data.hint;
  if (isAnswerInTargetLanguage) {
    Speaker.speak(document.config.targetLanguage, answer);
  }
  var isCorrect = this.isCorrect(answer);
  if (isCorrect) {
    if (!isAnswerInTargetLanguage) {
      Speaker.speak(document.config.targetLanguage, this.practicedWord);
    }
    this.closeAnswered();
    updateProgressBars();
  } else {
...
OpenQuestion.prototype.guess = function (answer) {
  var isAnswerInTargetLanguage = this.practicedWord !== this.data.hint;
  if (isAnswerInTargetLanguage) {
    Speaker.speak(document.config.targetLanguage, this.correct[0]);
  } else {
    Speaker.speak(document.config.targetLanguage, this.practicedWord);
  }
  this.closeAnswered();
...
var EnglishOnMenu = new function () {
  ...
  this.addToggleSwitch = function (cls, toggle) {
    toolbar.append($('<div>').addClass('eo-toggle').addClass('eo-toolbar-toggle').addClass(cls).append($('<div>').addClass('eo-toggle-inner').click(function (e) {
      e.preventDefault();
      toggle();
    })));
  };

  this.addToggleButton = function (cls, toggle) {
    toolbar.append($('<div>').addClass('eo-toolbar-button').addClass('eo-toolbar-button-' + cls).click(function (e) {
      e.preventDefault();
      toggle();
    }));
  };
  ...
  this.show = function () {
    ...
    this.closeHandler = function (e) {
      if (!this.isInProcessOfShowing && this.container.has(e.target).length === 0) {
        this.hide();
      }
      this.isInProcessOfShowing = false;
    }.bind(this);
    // because the handler will fire for the click that opened the menu
    this.isInProcessOfShowing = true;
    $(document).on('click', this.closeHandler);

    this.container.addClass('eo-visible');
  };

  this.hide = function () {
    this.container.removeClass('eo-visible');
  ...
  this.toggle = function (show) {
    if (show == null) {
      show = !this.container.hasClass('eo-visible');
    }
    if (show) {
      this.show();
    } else {
  ...
  this.hide();
  this.container.appendTo(EnglishOnButton.element);
}();
